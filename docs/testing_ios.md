...
> **Phase 4 tests 24–27 superseded by Phase 5.** Tests 24–27 drive ✕ visibility via Keychain token injection (`--uitest-inject-delete-token`). Once Phase 3 of `refactor_video_player_page_delete_eligibility.md` ships, ✕ visibility is driven by the server's `can_delete` field and the token injection path is removed from `showDeleteButton`. Tests 24–27 must be updated or replaced when Phase 3 ships.
...
| `GH_TEST_UPLOADER_USER` | `uploader` | HTTP username for the uploader account. Required only for `testAuthDelete403ClearsToken`. The uploader path in `delete_media_files.php` validates the token hash and returns 403 for an invalid token; the admin path uses `asset_ids[]` payload, receives 400 for the app's scalar request, and never reaches the 403 branch. See P11 in `problem_ios_testing_media_player_unification.md`. |
...
| Test | Needs credentials | What it verifies |
|---|---|---|
| `testAuthDeleteButtonAbsentWithoutToken` | Yes | Waits for at least one `unified_list_video_cell` to confirm the list has loaded, then asserts no `unified_list_delete_button` exists. Cell-wait is required — an unloaded list trivially has no delete button, producing a false pass. P9 cleanup guarantees the token store is empty. |
| `testAuthDeleteButtonVisibleForOwnUpload` | Yes + `GH_TEST_DELETE_FILE_ID` | `unified_list_delete_button` visible on the card whose `id` matches the injected file ID; absent on all other cards |
| `testAuthDeleteConfirmDialogAppears` | Yes + `GH_TEST_DELETE_FILE_ID` | Tapping `unified_list_delete_button` raises the delete confirm alert; assert `app.alerts["Delete your video?"].waitForExistence(timeout: 10)` — title must be specific to rule out error alerts; tap Cancel — no API call, server state unchanged |
| `testAuthDelete403ClearsToken` | Yes + `GH_TEST_DELETE_FILE_ID` + `GH_TEST_UPLOADER_USER` + `GH_TEST_UPLOADER_PASS` | Confirming delete with an invalid token triggers a 403 → assert any error alert with `app.alerts.firstMatch.waitForExistence(timeout: 25)` (network round-trip; 25 s budget); relaunch without injection shows no delete button. Uses `launchAuthListWithToken(injectToken: true, useUploader: true)` and `requireUploaderCredentials()` (throws `XCTSkip` if `GH_TEST_UPLOADER_USER`/`GH_TEST_UPLOADER_PASS` absent). **Precondition 1: must use uploader account.** `delete_media_files.php` routes by HTTP auth user: `admin` expects `asset_ids[]` (array payload); the app sends `asset_id` (scalar), so `admin` receives 400 — the 403 Keychain-cleanup branch never fires and the relaunch assertion fails. Kept in separate vars (`GH_TEST_UPLOADER_USER`/`GH_TEST_UPLOADER_PASS`) so other tests continue to run as admin without reconfiguration. **Precondition 2: dev server must be reachable.** A connection error shows an error alert but skips Keychain cleanup; distinguish via `[UnifiedList] deleteAuthenticated 403` vs `[UnifiedList] deleteAuthenticated error` in console. |
...
Add UI tests as `// MARK: - Phase 5 — Delete Eligibility` in `GigHiveUITests.swift`. Add unit tests as `// MARK: - Phase 5 — Delete Eligibility` in `GigHiveTests.swift` (or a dedicated `DeleteEligibilityTests.swift` file in the `GigHiveTests` target).
...
| Test | File | Needs credentials | What it verifies |
|---|---|---|---|
| `testDelEligUploaderNoDeleteButtonForGuestUpload` | `GigHiveUITests.swift` | `GH_TEST_HOST` + `GH_TEST_UPLOADER_USER` + `GH_TEST_UPLOADER_PASS` | Signs in as `uploader`, navigates to unified list, waits for ≥ 1 `unified_list_video_cell`, then asserts `unified_list_delete_button` does not exist anywhere in the list. **Fixture precondition:** the test dev server must contain only guest-uploaded assets for this uploader account — no authenticated uploads — so the server returns `can_delete: false` for every entry. If the uploader has any authenticated uploads on the test server, those would receive `can_delete: true` and produce a ✕, breaking the assertion. See precondition note in `refactor_video_player_page_delete_eligibility.md` Phase 3 tests. |
| `testDelEligAdminSeesDeleteButton` | `GigHiveUITests.swift` | `GH_TEST_HOST` + `GH_TEST_USER` (admin) + `GH_TEST_PASS` | Signs in as `admin`, navigates to unified list, waits for ≥ 1 `unified_list_video_cell`, asserts `unified_list_delete_button` exists on at least one cell. Server returns `can_delete: true` for admin on all entries. |
| `testDelEligMissingCanDeleteDecodesAsFalse` | `GigHiveTests.swift` | None — pure unit test | Decodes a synthetic `MediaEntry` JSON string that omits the `can_delete` field. Asserts the resulting `entry.canDelete == false` with no crash or decode error. Validates backward compatibility with pre-Phase-2 server responses. |
| `testDelEligGuestUploadDoesNotWriteTokenStore` | `GigHiveTests.swift` | None — pure unit test | Calls the guest upload finalize code path in isolation (injecting a mock finalize response with a `delete_token`). Asserts `UploaderDeleteTokenStore.load(host: "test.host")` returns an empty array — confirming the guest path never writes to the authenticated token store. |
