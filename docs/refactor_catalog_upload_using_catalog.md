...
Ready — Phase 1 (`docs/feature_completed_db_catalog_insert.md`) is complete and stable as of June 2026.
...
- **Step 1 — Window-shop, don't buy:** You point the catalog at a folder on your server. It reads only filenames and sizes — like reading a menu — without opening, hashing, or uploading anything. → `admin/admin_database_catalog_media_from_folder.php` (UI) + `admin/catalog_scan_start.php` (endpoint)
...
- **Step 4 — Pick the folder again:** The browser can't hold onto your files between sessions, so the promote UI asks you to pick the same folder(s) again. It tells you exactly which folder name to pick for each source root. → `admin/admin_database_catalog_promote.php` (same page, now presenting the folder picker)
...
## User Journeys: Phase 1 vs Phase 2 — What Each Phase Does
...
Catalog scans (one destructive + zero or more non-destructive adds)
  ...
  → POST catalog_promote_writeback.php { path_hash, upload_job_id } per file → asset_id resolved server-side
...
Pre-filtering is the primary gain: on a 500-file folder where 300 are unwanted, that is a 60% reduction in browser hashing and upload work. Metadata (`org_name`, `event_date`, `event_type`, `location`, `label`, `participants`) is pre-populated from catalog entries — no re-entry required. Per-entry fields that are NULL fall back to the parent `catalog_scans` row at promote time (see Promote Implementation Notes below).
...
- Phase 1 of `docs/feature_completed_db_catalog_insert.md` is complete and stable.
...
`catalog_entries.source_relpath` uses the table default `utf8mb4_unicode_ci` — **not** `utf8mb4_bin`. An earlier design used `utf8mb4_bin` (byte-exact) but it was reverted; see the decision log in `docs/feature_completed_db_catalog_insert.md` for the full rationale. Key reasons: `_bin` broke cross-OS deduplication (macOS stores paths as NFD; Linux uses NFC — `_bin` treats them as different even when visually identical) and created JOIN inconsistencies with `assets.source_relpath`.
...
- `catalog_promote_start.php` must include `path_hash` in each manifest row. The promote UI uses this to build a client-side `source_relpath → path_hash` map. When `import_manifest_upload_finalize.php` returns `200` for a file, the UI looks up that file's `path_hash` from the map and POSTs it to `catalog_promote_writeback.php`.
- After `ingestComplete()` completes for a file, the browser POSTs `{ path_hash, upload_job_id }` to `catalog_promote_writeback.php` — **do not include `asset_id` in the browser payload**; the browser has no direct access to the server-assigned `asset_id`.
...
- The write-back resolves `asset_id` server-side:
```sql
SELECT a.asset_id
FROM assets a
JOIN catalog_entries ce ON ce.source_relpath = a.source_relpath
WHERE ce.path_hash = ?
