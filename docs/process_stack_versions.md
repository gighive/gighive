...
The task uses three native Ansible modules on the controller (`delegate_to: localhost`):
1. `ansible.builtin.stat` — checks if `VERSION` exists
2. `ansible.builtin.slurp` — reads `VERSION` if present (base64-decoded + trimmed)
3. `ansible.builtin.command` (`git rev-parse --short HEAD`) — fallback when `VERSION` is absent

A final `set_fact` selects the result via Jinja2 ternary, falling back to `'unknown'` if both fail.

> **Note:** `lookup('pipe', ...)` was investigated and rejected — it returns Python `None` rather than an empty string on some Ansible versions, causing `| trim` to coerce it to the string `"None"` (truthy), silently defeating the fallback chain.
