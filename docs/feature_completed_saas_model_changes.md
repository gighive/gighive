...
16. Build superadmin/operator console — platform-level tenant list, usage metrics, suspend/reactivate controls; **two-tier admin structure (see note below)**
   ...
   >
   > GigHive SaaS has two distinct admin tiers that must not be conflated:
   >
   > **Tier 1 — Platform superadmin (GigHive operator, `role = superadmin`)**
   > The platform owner (GigHive itself) is the superuser. The step 16 operator console is their exclusive view — it is cross-tenant, accessible at a reserved path such as `gighive.app/superadmin` (never at a tenant subdomain), and requires `role = superadmin`. From here the operator can see every tenant's billing status, storage consumption, and suspension state, and can take administrative action (suspend, reactivate, delete). The superadmin is assigned `tenant_id = 1` (the seed operator tenant) in the `users` table by convention (see Schema Changes, step 3 note), but the RBAC middleware must grant cross-tenant visibility regardless of the row's own `tenant_id`.
   >
   > **Tier 2 — Tenant owner (`role = owner`) monitoring their own mini-GigHive**
   > Every tenant's owner has a self-service admin view scoped to their own tenant. This is the "mini-GigHive" experience: they see their own events, galleries, billing/subscription status, storage usage, and contributor list — nothing from any other tenant. The subdomain they were assigned at signup (e.g., `theirband.gighive.app`, provided by Cloudflare wildcard DNS and the step 6 resolver) provides the `tenant_id` for every request; the existing admin pages (`admin_system.php` and related) must scope all queries and views to `AuthContext::tenantId()` when `SAAS_MODE=true`. Step 12 builds the owner admin shell; this step (16) builds the superadmin-only cross-tenant console.
   >
