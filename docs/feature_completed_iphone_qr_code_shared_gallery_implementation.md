...
- **Step 2a** — Swift: add optional **Clip label** field to the upload form
  ...
  - `UploadPayload+GuestUpload.swift`: `forGuestUpload(fileURL:eventDetails:displayName:clipLabel:)` — trim `clipLabel`; if blank fall back to `"Untitled clip"`; pass resolved string as `label` in `UploadPayload`. **Do not** use `fileURL.lastPathComponent` as the label — iOS exports produce UUID-named temp files which are meaningless to users. **Do not** use a date/time fallback — long date strings cause row height growth under large accessibility font sizes and add no useful information to the guest gallery view.
...
- **Timestamp semantics — `upload_jobs` vs `assets`:** for the shared gallery, `upload_jobs` is the operational workflow record and `assets` is the durable media-library record. `upload_jobs.started_at` marks when the guest upload workflow began, so it is useful for chronological ordering and operational debugging but it is **not** the canonical long-term timestamp for the file. If a canonical persistent timestamp is needed for the file record after ingestion, prefer `assets.created_at` (or `assets.media_created_at` when the product explicitly wants the media's original capture time rather than library-ingest time). This distinction also means gallery fallback titles should stay short and semantic — e.g. `Untitled clip` when `label` is blank — instead of embedding upload date/time into the visible title text.
- Response (iOS path):
  ```json
  { "status": "approved",
    "days_remaining": 87,
