# Refactor follow-ups for Admin Sections A/B — Import Media from Folder (Deferred)

This document captures the two refactor steps that were discussed but intentionally deferred after completing the "minimal dedupe" and helper extraction work for these sections (formerly Sections 4/5 in `admin.php`, now moved to `admin_database_load_import_media_from_folder.php`).

Scope reference:
- `ansible/roles/docker/files/apache/webroot/admin_database_load_import_media_from_folder.php`
- Sections:
  - Section A: "Reload Database from Folder" (destructive reload)
  - Section B: "Add to Database from Folder" (non-destructive add)

## Step 4 (Deferred): UI controller abstraction

### Goal
Create a small abstraction that centralizes the DOM wiring and UI transitions for Section A and Section B so the runner logic no longer manually tweaks button text/styles and status HTML in multiple places.
...
### Why it's helpful
- Reduces the risk of Section A and Section B drifting in behavior/UI.
...
## Step 5 (Deferred): Move JS out of `admin_database_load_import_media_from_folder.php` into a separate file

### Goal
Extract the large inline `<script>` block into a standalone file (e.g., `admin_scan_import.js`) and include it from `admin_database_load_import_media_from_folder.php`.

### Why it’s helpful
- Improves maintainability/readability of `admin_database_load_import_media_from_folder.php`.
...
- Keep endpoint fetch paths relative to `admin_database_load_import_media_from_folder.php` location (as they are now).
- Ensure Ansible role copies the new `.js` file to the same webroot.

### Recommended validation
- Load `admin_database_load_import_media_from_folder.php` in browser devtools:
  - Confirm no 404 for the JS file.
  - Confirm no JS errors.
- Exercise Section A and Section B flows (hashing, stop, clear-cache, success banners).
